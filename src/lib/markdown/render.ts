...
function developersMarkdown(abs: Abs): MarkdownDoc {
  ...
  return {
    ...
    body: tidy([
      ...
      '## Rate limits',
      '',
      `MCP endpoints allow **${READ_LIMIT.limit} requests a minute** per client; \`/ask\` allows **${ASK_LIMIT.limit}**, because each call scans the whole corpus. Successful \`/ask\` responses carry \`RateLimit-Remaining\`; a \`429\` carries \`Retry-After\`. The MCP endpoints and the discovery documents send them too.`,
      '',
